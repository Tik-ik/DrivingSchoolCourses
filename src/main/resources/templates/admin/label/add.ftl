...
<title>${siteName!""}|分类标签管理-添加分类标签</title>
    ...
    <main class="lyear-layout-content">
      <div class="container-fluid">
        <div class="row">
          <div class="col-lg-12">
            <div class="card">
              <div class="card-header"><h4>添加分类标签</h4></div>
              <div class="card-body">
                <form action="add" id="label-add-form" method="post" class="row">
                  <div class="input-group m-b-10">
                    <span class="input-group-addon">分类标签名</span>
                    <input type="text" class="form-control required" id="name" name="name" value="" placeholder="请输入分类标签名" tips="请填写分类标签名" />
                  </div>
	...
	$("#add-form-submit-btn").click(function(){
		...
		$.ajax({
			...
			success:function(data){
				if(data.code == 0){
					showSuccessMsg('分类标签添加成功!',function(){
